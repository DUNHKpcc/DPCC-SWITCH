...
const SQL_EXPORT_HEADER: &str = "-- DPCC-SWITCH SQLite Export";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        if trimmed.starts_with(SQL_EXPORT_HEADER) || trimmed.starts_with(LEGACY_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 DPCC-SWITCH 导出的 SQL 备份文件。",
            "Only SQL backups exported by DPCC-SWITCH are supported.",
        ))
    }
